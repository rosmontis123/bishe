...
class QComboCheckBox(QComboBox):
    ...
    def __init__(self, tool_bar, np_list=None, parent: QWidget = None):
        super().__init__(parent)
        self.vars = dict()
        self.np_list = np_list
        self.tool_bar = tool_bar
        self.list1 = []
        if self.np_list is not None:
            for i in range(len(tool_bar)):
                if i not in self.np_list:
                    self.list1.append(True)
                else:
                    self.list1.append(False)
        # print(self.list1)
        ...
        # self.setStyleSheet('''width = 300px;height = 100px''')
    ...
    def add_item(self, text: "str"):
        ...
        if text != '(全选)'and len(self.list1) != 0:
            index = self.tool_bar.index(text)
            # print(index)
            if self.list1[index]:
