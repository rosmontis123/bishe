...
class CheckableComboBox(QComboBox):
	def __init__(self, parent, items, filter, forbids):
	...
	def addItem(self, text, data=None):
		...
		if '-' in text:
			g1, g2 = text.split('-')
			if g1 in self.forbids and (g2 == 'REF' or g2 == 'AV' or g2 in self.forbids):
				item.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
			else:
				item.setFlags(~(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable))
				self.model().appendRow(item)
				return
		elif text in self.forbids:
			item.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
	...
	def show_selected(self):
		self.text.clear()
		t = "已选" + str(len(self.checkedItems)) + '项，' + '共' + str(self.count()) + '项'
		self.text.setText(t)
